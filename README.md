...
- Bash shell (required - do not use `sh`)
...
# Run setup (requires bash and sudo)
sudo bash anon-evader.sh
# OR make executable and run directly
# chmod +x *.sh && sudo ./anon-evader.sh
```
...
bash proxy-runner.sh curl -s ifconfig.me
bash proxy-runner.sh -p torsocks nmap -sS target.com
bash proxy-runner.sh -t wget http://example.com  # Test first
